...
class TransactionPolicy:                            #Strategy
    def can_perform_transaction(self, user):
        pass

class BasicTransactionPolicy(TransactionPolicy):
    MAX_TRANSACTIONS_PER_DAY = 5

    def can_perform_transaction(self, user):
        current_date = date.today()
        if user._last_transaction_date != current_date:
            user._transaction_count = 0
            user._last_transaction_date = current_date
        if user._transaction_count >= self.MAX_TRANSACTIONS_PER_DAY:
            print("Transactions for the day exceeded. Do you want to upgrade to Premium?")
            print("1. Yes 2. No")
            choice = input("Enter choice (1 or 2): ")
            if choice == '1':
                new_user, success = user.upgrade_to_premium()
                if success:
...
class PremiumTransactionPolicy(TransactionPolicy):
    def can_perform_transaction(self, user):
        return True, user
...
class User:
    ...
    def can_perform_transaction(self):
        return self._transaction_policy.can_perform_transaction(self)
...
class BasicUser(User):
    def __init__(self, username, initial_balance, db: Database = None):
        super().__init__(username, initial_balance, db)
        self._transaction_policy = BasicTransactionPolicy()
...
class PremiumUser(User):
    def __init__(self, username, initial_balance, db: Database = None):
        super().__init__(username, initial_balance, db)
        self._transaction_policy = PremiumTransactionPolicy()
